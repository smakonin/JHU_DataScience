...
png(file = "plot14.png", bg = "white")

par(mfrow = c(2, 2))

plot(subds$dt, subds$Global_active_power, type = "n", xlab = "", ylab = "Global Active Power (kilowatts)") 
lines(subds$dt, subds$Global_active_power) 

plot(subds$dt, subds$Voltage, type = "n", xlab = "datetime", ylab = "Voltage") 
lines(subds$dt, subds$Voltage)
...
legend("topright", legend = names(subds)[7:9], col = c("black", "red", "blue"), lty=1, lwd=1.0, box.lwd = 0)

plot(subds$dt, subds$Global_reactive_power, type = "n", xlab = "datetime", ylab = "Global_reactive_power") 
lines(subds$dt, subds$Global_reactive_power) 
